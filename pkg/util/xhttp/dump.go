...
func (m *DumpTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if mode.IsDumpRequest() {
		b := m.dumpRequest(req, mode)
		_, _ = fmt.Fprintf(m.writer(), "%s\n", string(b))
	}
	...
	if mode.IsDumpResponse() {
		...
		b := m.dumpResponse(resp, mode, elapsed)
		_, _ = fmt.Fprintf(m.writer(), "%s\n", string(b))
	}
	return resp, err
}
...
func (m *DumpTransport) dumpRequest(req *http.Request, mode DumpMode) []byte {
	buf := &bytes.Buffer{}
	...
	if err != nil {
		_, _ = fmt.Fprintf(buf, "failed to dump request: %v\n", err)
	} else {
		_, _ = fmt.Fprintf(buf, "%s\n", string(b))
	}
...
func (m *DumpTransport) dumpResponse(resp *http.Response, mode DumpMode, elapsed time.Duration) []byte {
	req := resp.Request
	buf := &bytes.Buffer{}
	...
	if err != nil {
		_, _ = fmt.Fprintf(buf, "failed to dump response: %v\n", err)
	} else {
		_, _ = fmt.Fprintf(buf, "%s\n", string(b))
	}
