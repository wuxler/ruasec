...
import (
	...
	"github.com/wuxler/ruasec/pkg/ocispec/manifest/dockerschema2"
)
	...
	testcases := []struct {
	...
	}{
		{
			...
			references: []imgspecv1.Descriptor{
				// config
				{
					MediaType: ocispec.MediaTypeDockerV2S2ImageConfig,
					Size:      7023,
					Digest:    "sha256:b5b2b2c507a0944348e0303114d8d93aaaa081732b86451d9bce1f432a537bc7",
				},
				// layers
				{
					MediaType: ocispec.MediaTypeDockerV2S2ImageLayerGzip,
					Size:      32654,
...
func TestUnmarshalManifestList(t *testing.T) {
	testcases := []struct {
	...
	}{
		{
			input: "v2s2.manifest.list.json",
			desc: imgspecv1.Descriptor{
				MediaType: ocispec.MediaTypeDockerV2S2ManifestList,
				Size:      1797,
	...
	for _, tc := range testcases {
		t.Run(tc.input, func(t *testing.T) {
			...
			indexmnft, ok := mnft.(ocispec.IndexManifest)
			require.True(t, ok)
