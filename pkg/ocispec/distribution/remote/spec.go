...
import (
	...
	"github.com/wuxler/ruasec/pkg/ocispec/remote"
	"github.com/wuxler/ruasec/pkg/util/xhttp"
	"github.com/wuxler/ruasec/pkg/util/xio"
)

var (
	_ distribution.Spec = (*Spec)(nil)
)

// NewSpec creates a client which implements distribution-spec interface to the remote
...
func (spec *Spec) StatManifest(ctx context.Context, repo string, reference string) (imgspecv1.Descriptor, error) {
	...
	request.Header.Set("Accept", remote.ManifestAcceptHeader())
	...
	desc, err := remote.DescriptorFromResponse(resp, dgst)
	if err != nil {
		return zero, err
	}
...
func (spec *Spec) doGetManifestRequest(ctx context.Context, repo string, reference string) (*http.Response, error) {
...
func (spec *Spec) StatBlob(ctx context.Context, repo string, dgst digest.Digest) (imgspecv1.Descriptor, error) {
	...
	return remote.DescriptorFromResponse(resp, dgst)
}
...
func (spec *Spec) pushManifest(ctx context.Context, repo string, desc imgspecv1.Descriptor, content []byte, ref string) error {
...
func (spec *Spec) doGetUploadStatusRequest(ctx context.Context, repo string, id string) (*http.Response, error) {
	ctx = authn.AppendScopes(ctx, authn.RepositoryScope(repo, authn.ActionPull, authn.ActionPush))
	url := spec.endpoint(fmt.Sprintf("/v2/%s/blobs/uploads/%s", repo, id))
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, url, http.NoBody)
	if err != nil {
...
func (spec *Spec) completePushAfterPost(ctx context.Context, postRequest *http.Request, postResp *http.Response, getter cas.ReadCloserGetter) error {
	// monolithic upload
...
func (spec *Spec) extractReferrersDescriptors(resp *http.Response, artifactType string) ([]imgspecv1.Descriptor, error) {
	// parse response body
...
// api endpoint.
func (spec *Spec) doGetReferrersRequest(ctx context.Context, repo string, dgst digest.Digest, artifactType string) (*http.Response, error) {
	ctx = authn.AppendScopes(ctx, authn.RepositoryScope(repo, authn.ActionPull))
...
func (it *repoIterator) Next(ctx context.Context) ([]string, error) {
	...
	next, err := remote.GetNextPageURL(resp)
	if err != nil {
