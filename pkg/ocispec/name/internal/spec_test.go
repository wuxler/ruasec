...
func TestFullReferenceRegexp(t *testing.T) {
	t.Parallel()
	if AnchoredReferenceRegexp.NumSubexp() != 3 {
		t.Fatalf("anchored reference regexp should have three submatches: %v, %v != 3",
			AnchoredReferenceRegexp, AnchoredReferenceRegexp.NumSubexp())
	}
	...
	for _, tc := range testcases {
		...
		t.Run(tc.input, func(t *testing.T) {
			t.Parallel()
			checkRegexp(t, AnchoredReferenceRegexp, tc)
		})
	}
}
